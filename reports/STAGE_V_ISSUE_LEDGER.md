...
| SV-004 | Dead code | P3 | `apps/web/_backup/page_bootstrap.tsx.bak` 备份残留 | apps/web/_backup | OPEN（记录） | 建议移 artifacts/删除（真人确认后） | 非运行路径 | 无（不影响运行） |
| SV-005 | Dead code | P3 | `scripts/_patch_ga2*.py` 一次性补丁（ga2b 陈旧语法） | scripts/ | OPEN（记录） | 归档/修正（不影响运行，不在测试收集路径） | 非运行路径 | 无 |
...
**统计：P0=0 / P1=0（剩余）/ P2=0（剩余）/ P3=3（记录收口）/ INFO=4（记录）/ SHOULD_FIX=1（记录）**
