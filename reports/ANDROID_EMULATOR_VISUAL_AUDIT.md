...
| 04 Timeline | `04_timeline.png`（397 KB） | 正常 | Life Stream：类型/时间/演员/来源（溯源可见）；筛选 chips | 事件类型 `pet.asked` 等显示原文 | P2 | `ui_labels.tsx` 补中文标签 | 已修（最终构建设备端验证："提问"标签可见） | PASS |
...
关键 P2 修复状态：
  · pet.asked/social.* 中文标签 —— 最终构建设备端验证通过（时间线显示"提问"，见 after/04_timeline.png）
  · 登出清理宠物上下文 —— 最终构建设备端验证通过（登出后 Today 回到"宠物/查看宠物档案"，无串数据）
