...
/** PLI pro (Vet / Trainer / Service). `output: standalone` for lean production
 *  Docker image; `basePath` is optional — set via NEXT_BASE_PATH when deployed
 *  behind a reverse-proxy path prefix. Empty → root.
 ...
 *  (Same guard as apps/web — apps/admin omits it but pro must pass the
 *  Windows local-build gate.) */
