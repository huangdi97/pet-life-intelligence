...
test("STAGE-V-VISUAL-01 capture baseline screenshots at 5 widths x 12 pages", async ({ page, request }) => {
  ...
  for (const width of WIDTHS) {
    ...
    for (const def of PAGES) {
      ...
      // (administered_at / due_at / onset_at ...) and per-seed UUIDs
      // (behavior_event_id / task_id / dose_id / source_ref ...), rendered as
      // plain text in timeline rows. Both change on every re-seed, so without
      // normalization any two runs diff there. Normalize date-like and
      // UUID-like substrings in text nodes to fixed tokens (same character
      // width family) — capture & compare then stay reproducible regardless
      // of the wall clock or generated ids at seed time.
      await page.evaluate(() => {
        const dateRe =
        ...
        while ((node = walker.nextNode() as Text | null)) {
          ...
          uuidRe.lastIndex = 0;
          const nv = v
            .replace(dateRe, "2026-01-01T00:00:00")
            .replace(cnDateRe, "1月1日")
            .replace(uuidRe, "00000000-0000-0000-0000-000000000000");
