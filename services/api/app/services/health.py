...
Rule engine output can never be lowered by AI (GOAL §11.3); AI results are
provenance-tagged AI_DERIVED (PLI-052/053/054).
generate_vet_brief lives in app.services.health_vet_brief and is re-exported
here so `from app.services import health` keeps working unchanged.
"""
...
from app.models import AIInferenceLog, HealthEvent, Observation, Pet, TriageAssessment
from app.services.ai_gateway import get_gateway
from app.services.eventlog import create_life_event, create_notification
from app.services.health_vet_brief import generate_vet_brief

__all__ = [
    "add_ai_observations",
    "collect_red_flag_texts",
    "generate_vet_brief",
    "log_ai_inference",
    "run_rule_triage",
]
...
# SAFETY:
# run_rule_triage is the deterministic rule-engine path. LLM/AI output must
# never downgrade its level: levels only ever increase within one health
# event (max_level guard below), and add_ai_observations re-runs this engine
# after AI extraction so AI presence can never lower the triage.
